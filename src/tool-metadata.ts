...
export function createToolDescription(maxOutputBytes: number): string {
  return [
    ...
    "})",
    "",
    "Change kinds: replace, delete, insertBefore, insertAfter, replaceFile, deleteFile. An optional end anchor extends replace/delete. Create a missing file with revision: null and one replaceFile change. After a successful edit, discard every prior revision and anchor for that file; batch compatible same-file changes in one edit or re-read before the next, and never mutate the same file concurrently. On mismatch, re-read before retrying. Raw mode is for JSON or other machine parsing. Missing offset means 1, missing totalLines means lines, and missing hasMore/truncated means false.",
    "",
