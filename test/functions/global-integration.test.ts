...
describe("global functions", () => {
  it("loads globals, supports project dependencies, and applies scope precedence", async () => {
    ...
    await writeGlobalFunction(
      "globalConsumer",
      "/** Global consumer. */ async function globalConsumer({ sharedValue }) { return sharedValue(); }",
    );
    ...
    await expect(value("async ({ sharedValue }) => sharedValue()")).resolves.toBe("global");
    await expect(value("async ({ globalConsumer }) => globalConsumer()")).resolves.toBe("global");
    await expect(value("async ({ projectConsumer }) => projectConsumer()")).resolves.toBe("global");
    ...
    await expect(value("async ({ globalConsumer }) => globalConsumer()")).resolves.toBe("project");
    ...
    await expect(value("async ({ globalConsumer }) => globalConsumer()")).resolves.toBe("session");
    await expect(value("async ({ projectConsumer }) => projectConsumer()")).resolves.toBe(
      "session",
    );
    const metadata = await value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => getSaved("sharedValue")',
    );
    ...
    const global = await value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => getSaved("sharedValue", "global")',
    );
  ...
  it("honors a trusted project opt-out", async () => {
    ...
    await expect(value("async ({ hiddenGlobal }) => hiddenGlobal()")).rejects.toThrow(
      /Property 'hiddenGlobal' does not exist/,
    );
    await expect(
      value(
        "async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => listGlobal()",
      ),
    ).rejects.toThrow("Global functions are disabled");
  });
  ...
  it("promotes and removes a global function with confirmation", async () => {
    ...
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => promote("portableHelper", "Portable helper.", { to: "global" })',
      ),
    ...
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => removeGlobal("portableHelper")',
      ),
  ...
  it("blocks promotion while non-global dependencies remain", async () => {
    ...
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => promote("dependentPortable", "Dependent helper.", { to: "global" })',
      ),
  ...
  it("validates global capability options and interactive confirmation", async () => {
    ...
    ).resolves.toEqual([expect.objectContaining({ name: "confirmedGlobal" })]);
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => getGlobal("confirmedGlobal")',
      ),
    ).resolves.toEqual(expect.objectContaining({ source: expect.not.stringContaining("@pit") }));
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => getGlobal("missingGlobal")',
      ),
    ).rejects.toThrow("is unavailable");
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => getSaved("missingGlobal", "global")',
      ),
    ).rejects.toThrow("Global saved function");
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => (promote as any)("confirmedGlobal", "Summary", { unknown: true })',
      ),
    ).rejects.toThrow("unknown fields");
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => (promote as any)("confirmedGlobal", "Summary", { to: "machine" })',
      ),
    ...
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => removeGlobal("confirmedGlobal")',
        cancelled,
      ),
  ...
  it("blocks global removal while global dependents remain", async () => {
    ...
    await writeGlobalFunction(
      ...
      "/** Removal consumer. */ async function globalRemovalConsumer({ globalRemovalBase }) { return globalRemovalBase(); }",
    );
    await sessionStart({}, context());

    const plan = await value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => planRemoval("globalRemovalBase", "global")',
    );
    ...
    await expect(
      value(
        'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => removeGlobal("globalRemovalBase")',
      ),
...
it("requires confirmation before global promotion", async () => {
  ...
  await expect(
    value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => promote("confirmationHelper", "Confirmed helper.", { to: "global" })',
      cancelled,
    ),
  ).rejects.toThrow("cancelled");
  const noUi = context({ hasUI: false });
  ...
  await expect(
    value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => promote("confirmationHelper", "", { to: "global" })',
    ),
...
it("reports sorted global metadata and transitive removal blockers", async () => {
  ...
  const plan = await value(
    'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => planRemoval("chainBase")',
  );
  expect(plan).toMatchObject({
  ...
  await expect(
    value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => planRemoval("missing", "global")',
    ),
...
it("handles cancelled and absent global manager operations", async () => {
  ...
  await expect(
    value(
      'async ({ functions: { getGlobal, getSaved, listGlobal, planRemoval, promote, removeGlobal } }) => removeGlobal("absentCapabilityFile")',
    ),
